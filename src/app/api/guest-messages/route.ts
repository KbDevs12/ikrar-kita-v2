...
export async function POST(req: Request): Promise<Response> {
  ...
  try {
    ...
    const parsed = guestMessageSchema.parse(body)
    ...
    const created = await prisma.guestMessage.create({
      data: {
        ...
        name: parsed.name,
        // The message is plain text; React renders it as text-children which
        // escapes by default. We do not need DOMPurify here.
        message: parsed.message,
        ipHash,
      },
    })

    return jsonCreated({ message: { id: created.id, name: created.name, message: created.message } })
  } catch (err) {
